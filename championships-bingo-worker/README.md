...
完整协议、状态机和故障边界见 [跨服拆分架构](../docs/bingo-remote-architecture.md)，64 人容量与配置依据见 [性能分析报告](../docs/bingo-64-player-performance-report.md)。
...
5. 一局结束后停止实例，恢复三维度快照或重建实例，再接受下一局。
...
- 先采用性能报告中的 view/simulation distance、4 个 tick thread 和实体保护线，再根据 spark/Folia region 数据逐项 A/B 调整。
