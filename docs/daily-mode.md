...
自由游玩是正式赛事系统之外的一层大厅编排。它复用原有游戏实例和地图配置，但队列、同行小队、临时队伍、比赛结果与纪录均使用独立生命周期；与正式赛事的积分、轮次和赛程表相互独立。代码和配置中的稳定标识仍使用 `DAILY`/`daily`。
...
- `daily_player_stats`：按玩家和游戏聚合场次、胜场；Bingo 额外保存连线数、完成任务总数和单场最多完成数，保存完成任务数、连线数和胜场等玩法数据。
- `daily_match_results`：每场每位玩家的独立战绩，包含该场积分及 Bingo 进度，`matchId + uuid` 保证幂等。
- `daily_player_records`：按游戏、地图、地图版本、规则版本和纪录类型存储最佳耗时。

Bingo 记录胜场、连线数、完成任务总数和单场最多完成数；AceRace 按地图记录最快单圈和最快完整三圈；龙蛋狂欢按率先完成两项末地进度的队伍记录胜负。单局积分随该场战绩写入 `daily_match_results`，自由游玩的积分体系与正式赛事分别管理。

自由游玩占位符统一使用 `%cc_daily_*%`，包括 `mode`、`party_leader`、`party_size`、`selected_game`、`queue_state`、`queue_players`、`countdown`、`active_game`、`active_map`、`match_id`、`games`、`wins`。旧占位符 `points`、`best` 保留以兼容旧版本，固定返回 `0`。这些占位符只读取快照或缓存，离线/空上下文返回稳定默认值，不在 PAPI 回调线程访问数据库。
