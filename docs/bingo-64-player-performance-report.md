...
本文面向部署和运维 ChampionshipsCore 远程 Bingo 的管理员，提供 64 名参赛者的参考压测结果、推荐配置和验收方法。文中的数值是容量规划起点，不是对任意硬件、地图或插件组合的性能保证。
...
该结果只能证明测试服务器在该模型下能够存活，不能直接判定为“64 人生产验收通过”：

- 区块异步加载 P95/P99 约为 30 秒，不符合高速移动时的玩家体验要求；
- 布局切换使已加载实体数在约 2,000–21,425 之间振荡，没有证明 10,000 个实体可以严格稳态运行；
- 主要负载曾集中于一个 Folia region，总 CPU 仍有空闲时局部 TPS 也可以先失速；
- 逻辑加载者不会产生真实玩家的网络、实体追踪、背包、UI、Redis 和积分重放开销；
- 64 人、每队最多 4 人时应使用 16 队模型，而参考压测使用 8 个宏观锚点。

建议将结论分为两部分：

- **基础生存性通过**：12 分钟无崩溃、无加载/生成失败，分批清理可在数秒内完成；
- **生产验收未完成**：必须在预生成的干净世界上，使用 16 队、64 个真实协议客户端执行完整比赛与故障演练。
...
缺少这些条件时，不应直接将两次 TPS 或 MSPT 数值进行容量对比。
...
Paper 世界配置中的 `spawn-limits: -1` 和 `ticks-per-spawn: -1` 表示继承 `bukkit.yml`，不表示关闭自然刷怪。调整任何生成上限前，应先确认它的最终生效值。
...
参考测试曾观察到约 30% 的进程 CPU 峰值，但没有连续 CPU 时间序列。该数值只能辅助说明 region 并行度，不能当作精确容量指标。排障时应优先查看每个 region 的 TPS、MSPT、chunk 和 entity 数，而不是只看整个 JVM 的 CPU 平均值。
...
排障时应保存以下可重建信息，而不是把某台服务器的绝对路径或文件时间写入项目文档：
...
参考 12 分钟测试只满足时长、基础生存性、零加载/生成失败和清理标准；不满足区块延迟、严格实体窗口和真实 16 队客户端覆盖。
