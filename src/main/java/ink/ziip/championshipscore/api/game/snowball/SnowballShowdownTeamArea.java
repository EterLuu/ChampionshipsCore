...
public class SnowballShowdownTeamArea extends BaseMultiTeamGameInstance {
    ...
    public void resetArea() {
        ...
        if (world != null) {
            cleanEntities(world, boundingBox, Snowball.class);
        }
    }
    ...
    public synchronized void addShoot(Player assailant, Player player) {
    ...
    private synchronized void addTeamShootCount(ChampionshipTeam championshipTeam) {
        int times = teamShootTimes.merge(championshipTeam, 1, Integer::sum);
        if (times == 100) {
            endGame();
        }
    }
