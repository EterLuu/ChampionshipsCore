...
随 JAR 发布的默认配置用于短阶梯测试（8/32/64，目标 4,000/8,000/16,000）。长时间 10,000 实体稳定性测试应单独调整阶段时长和目标，并保留 TPS、失败数、磁盘和 pending 保护线。
...
该实现使用 8 个宏观锚点，无法精确表示 64 人、四人队制下的 16 队拓扑；也没有真实玩家的网络、背包、AI 仇恨和协议成本。因此 LoadTest 通过后，仍需使用协议客户端或真人完成整局远程 Bingo 验收。
