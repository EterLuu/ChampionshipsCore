...
import ink.ziip.championshipscore.authbridge.bridge.BridgeUuidResolver;
...
public final class ChampionshipsAuthBridgePlugin extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        AuthAdmissionOwner admissionOwner = AuthAdmissionOwner.parse(
                getConfig().getString("access.admission-owner", "PROXY"), AuthAdmissionOwner.PROXY);
        ...
        var synchronizer = new BridgeSynchronizer(this, client, new AuthMeHashStore(), state,
                ...
                new BridgeUuidResolver(Duration.ofSeconds(5), Duration.ofSeconds(10)), admissionOwner,
                getConfig().getString("access.banned-message", "&#ff6b26你已被服务器封禁。"));
        getServer().getPluginManager().registerEvents(new AccessListener(
            ...
            getConfig().getString("access.unbound-message", "&#ff6b26你还没有绑定 Minecraft 账号。"),
            getConfig().getString("access.banned-message", "&#ff6b26你已被服务器封禁。"),
            admissionOwner
        ), this);
